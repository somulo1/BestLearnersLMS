...
// node_modules/@react-spring/rafz/dist/react-spring_rafz.legacy-esm.js
...
// node_modules/@react-spring/shared/dist/react-spring_shared.legacy-esm.js
...
var createStringInterpolator2 = (config2) => {
  ...
  const interpolators = outputRanges.map((output2) => createInterpolator(__spreadProps(__spreadValues({}, config2), { output: output2 })));
  return (input) => {
    var _a;
...
// node_modules/@react-spring/core/dist/react-spring_core.legacy-esm.js
var import_react10 = __toModule(require_react());

// node_modules/@react-spring/animated/dist/react-spring_animated.legacy-esm.js
...
var withAnimated = (Component, host) => {
  ...
  return (0, import_react9.forwardRef)((givenProps, givenRef) => {
    ...
    return React.createElement(Component, __spreadProps(__spreadValues({}, usedProps), { ref }));
  });
};
...
function mergeConfig(config2, newConfig, defaultConfig) {
  if (defaultConfig) {
    defaultConfig = __spreadValues({}, defaultConfig);
    sanitizeConfig(defaultConfig, newConfig);
    newConfig = __spreadValues(__spreadValues({}, defaultConfig), newConfig);
  }
...
var SpringValue = class extends FrameValue {
  ...
  advance(dt) {
    ...
    anim.values.forEach((node2, i) => {
      if (node2.done)
        return;
      const to22 = node2.constructor == AnimatedString ? 1 : payload ? payload[i].lastPosition : toValues[i];
  ...
  start(to22, arg2) {
    let queue;
    if (!is.und(to22)) {
      queue = [is.obj(to22) ? to22 : __spreadProps(__spreadValues({}, arg2), { to: to22 })];
    } else {
      queue = this.queue || [];
      this.queue = [];
    }
...
function useSprings(length, props, deps) {
  ...
  const values = springs.map((x) => __spreadValues({}, x));
...
function useTransition(data, props, deps) {
  ...
  const ref = (0, import_react13.useMemo)(() => propsFn || arguments.length == 3 ? SpringRef() : void 0, []);
  ...
  const exitingTransitions = (0, import_react13.useRef)(new Map());
  ...
  each(transitions, (t, i) => {
    ...
    delay += trail;
    const payload = __spreadValues(__spreadProps(__spreadValues({}, defaultProps), {
  ...
  const renderTransitions = (render) => React22.createElement(React22.Fragment, null, transitions.map((t, i) => {
    ...
    return elem && elem.type ? React22.createElement(elem.type, __spreadProps(__spreadValues({}, elem.props), {
...
//# sourceMappingURL=chunk-UROOLUEL.js.map
