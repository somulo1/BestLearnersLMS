...
import { Box, Theme, CssMixBlendMode, Dimensions, MotionProps, PropertyAccessor, ValueFormat, ClosedCurveFactoryId, DotsItemSymbolComponent, SvgDefsAndFill } from '@nivo/core';
...
export type RadarCustomLayer<D extends Record<string, unknown>> = FunctionComponent<RadarCustomLayerProps<D>>;
...
export type RadarSvgProps<D extends Record<string, unknown>> = Partial<RadarCommonProps<D>> & RadarDataProps<D> & Dimensions & MotionProps & SvgDefsAndFill<RadarSvgFillMatcherDatum<D>> & RadarHandlers<D, SVGPathElement>;
export type BoundLegendProps = Required<Pick<LegendProps, 'data'>> & Omit<LegendProps, 'data'>;
export type MouseEventHandler<RawDatum, ElementType = HTMLCanvasElement> = (datum: RawDatum, event: React.MouseEvent<ElementType>) => void;
